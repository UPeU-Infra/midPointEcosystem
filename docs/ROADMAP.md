...
- **GAP-2 email — re-diagnosticado y enriquecido 2026-07-10** (reemplaza la cifra obsoleta "13,733 estudiantes sin CORREO_INST", que era solo un subconjunto de junio). Estado real en PROD tras el email-cleanup 2026-06-06:
  ...
  - **Resiliencia Koha para el backfill (2026-07-10, DT-9):** el import/backfill que materializa `sb:personalEmail` en egresados se suspendió porque al recomputar los 788 egresados que además son borrower Koha, el outbound a Koha caído (192.168.12.135, incidencia FortiGate 08-jul) lanzaba `CommunicationException` que escalaba a FATAL y abortaba el foco. Fix canónico aplicado en `koha-ils.xml`: `consistency/connectorErrorCriticality/network = partial`. Con Koha caído la proyección degrada a `partial_error` (auditado) y el foco sí materializa `personalEmail`. NO se usó `criticality` en la construction de los AR (no existe en el schema 4.10.2 — ver DT-9). Los 6 `AR-Koha-Patron` NO se modificaron.
