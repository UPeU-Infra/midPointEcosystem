...
| DT-8 | `AlreadyExistsException` Koha en CREATE para usuarios con patron existente de nombre duplicado (descubierto durante DT-7). Más de los 2 originales. | Koha ILS / datos | **Pendiente** — Hay usuarios en MidPoint cuyo nombre coincide con un patron Koha ya existente (cardnumber diferente). MidPoint intenta CREATE; Koha rechaza con 409. Requiere: (a) auditar todos los usuarios sin shadow Koha activo que tengan `PARTIAL_ERROR` en Koha, (b) actualizar cardnumber en Koha o ajustar correlación. **Nota 2026-07-10:** el `AlreadyExistsException` (409) es categoría de error `generic`, distinta del `network` de un Koha caído — DT-9 (resiliencia) NO lo enmascara: estos 409 deben seguir siendo visibles como `PARTIAL_ERROR`/fatal para forzar la remediación de datos. |
| DT-9 | Un recurso Koha **caído** (CommunicationException, categoría `network`) escalaba a FATAL y **abortaba el foco**: recomputar un egresado que además es borrower Koha con Koha 192.168.12.135:8001 caído (incidencia FortiGate 08-jul) tumbaba el clockwork y NO materializaba la proyección (extension `sb:personalEmail`, LDAP cache). Descubierto en GAP-2 (backfill egresados suspendido). | Resource `koha-ils.xml` (`consistency`) | ✅ **RESUELTO 2026-07-10** — Fix canónico MidPoint 4.10: `resource/consistency/connectorErrorCriticality/network = partial` (`ErrorSelectorType`→`CriticalityType`). Koha caído degrada a `partial_error` (VISIBLE/AUDITADO, **no** `ignore`) y el foco continúa. **Corrección de diseño:** la nota previa (2026-07-04) proponía `criticality=partial` en la `<construction>` de los 6 AR-Koha-Patron; se verificó contra `schema-4.10.2.jar` (contenedor PROD) que `criticality` NO existe en `ConstructionType` ni `AssignmentType` en 4.10.2 (solo en `ProvisioningScriptType`) → mecanismo inaplicable. El fix vive en el RESOURCE (una vez, cubre las 6 constructions), los 6 AR-Koha-Patron NO se tocaron. **Aclaración:** DT-9 es correcto y permanente, pero (a) NO era el bloqueante determinista del backfill GAP-2 — ese fue DT-10 (fila con identificador nulo); y (b) `connectorErrorCriticality=partial` por sí solo NO basta para un Koha totalmente caído: el conector Koha lanza un `IllegalArgumentException('Uid cannot be null')` (categoría interna, no `network`/`generic` degradable) durante el search de compensación → sigue siendo fatal en un recompute de dual (ver DT-11). Handling operacional canónico mientras .135 esté caído: Koha en `administrativeOperationalState/administrativeAvailabilityStatus = maintenance` (flag operacional transitorio vía REST PATCH, **NO** en el GitOps) → las proyecciones Koha quedan `pending` y el foco de los duales tiene éxito. **Verificado:** el backfill GAP-2 corrió bajo maintenance y materializó personalEmail para 2,517/2,667 duales sin abortar. **ACCIÓN PENDIENTE:** revertir Koha a `operational` cuando .135 recupere y correr `reconcile-koha-daily`. Ver `docs/runbooks/koha-maintenance-135-outage-2026-07-10.md`. |
| DT-11 | El conector `pe.upeu.connector.koha-http` (ScriptedREST) lanza `ConnectorException('Search script error') → IllegalArgumentException('Uid cannot be null')` cuando MidPoint invoca un search de compensación con Koha inalcanzable (No route to host). MidPoint no lo clasifica como error de conector degradable (`network`/`generic`), así que `connectorErrorCriticality` no lo puede bajar a `partial` → recompute de dual fatal si Koha está caído y NO en maintenance. | Conector `connector-koha` (repo `UPeU-Infra/connector-koha`) | **Pendiente (robustez de conector, fuera de este repo)** — El search script debería devolver vacío (o lanzar `ConnectorIOException`/`CommunicationException`, que SÍ es degradable) en vez de `IllegalArgumentException` cuando no hay Uid / el host es inalcanzable. Workaround operativo actual: maintenance mode (DT-9). |
| DT-10 | Backfill GAP-2 se suspendía **determinísticamente en progress 27**: el recurso `Oracle LAMB Egresados v3` es un conector **ScriptedSQL**; su `searchScript` construye `'__UID__': row.CODIGO`. En el full-search del import (sin filtro), al llegar a una fila con `CODIGO` NULL en `DAVID.VW_PERSONA_EGRESADO` (grupo nulo del `GROUP BY e.CODIGO`), el conector lanza `IllegalArgumentException(Uid cannot be null)` → falla el search fuente → task fatal. No lo cubre `connectorErrorCriticality` ni maintenance mode (es el recurso FUENTE, no Koha). Nunca se había expuesto porque el live-sync usa `EqualsFilter` por CODIGO, jamás un full-search. | Resource `oracle-lamb/egresados.xml` (`searchScript`) | ✅ **RESUELTO 2026-07-10** — Doble guarda en el `searchScript`: (a) SQL `WHERE e.CODIGO IS NOT NULL` antes del `GROUP BY` (elimina el grupo nulo en origen); (b) guarda en el loop `if (row.CODIGO == null) return;` (defensa). Un egresado sin identificador no debe importarse. Oracle LAMB sigue solo-lectura (el fix es en el conector, no en la vista). |
...
- **GAP-2 email — re-diagnosticado y enriquecido 2026-07-10** (reemplaza la cifra obsoleta "13,733 estudiantes sin CORREO_INST", que era solo un subconjunto de junio). Estado real en PROD tras el email-cleanup 2026-06-06:
  ...
  - **Backfill EJECUTADO 2026-07-10 (task `e9a2c7f0…`, `closed`, 30,917 procesados, `partial_error`):**
    - **alumni con `sb:personalEmail`: 2,959 → 23,622** (+20,663). Total users con `personalEmail`: 33,668 → 54,349.
    - Duales (alumni + shadow Koha): 2,517 / 2,667 obtuvieron `personalEmail` (94.4%) SIN abortar el foco (los ~150 restantes: sin `CORREO` en Oracle, irreducibles).
    - **El bloqueante real NO fue Koha sino DT-10:** el backfill se suspendía determinísticamente en progress 27 porque el `searchScript` ScriptedSQL de Egresados construía `__UID__` desde una fila con `CODIGO` NULL de `VW_PERSONA_EGRESADO` → `IllegalArgumentException('Uid cannot be null')`. Fix: `WHERE e.CODIGO IS NOT NULL` + guarda en loop.
    - **Resiliencia Koha (DT-9):** `koha-ils.xml` ahora tiene `consistency/connectorErrorCriticality/network=partial` + `generic=partial` (permanente, GitOps). Se descartó `criticality` en la `<construction>` de los AR: NO existe en el schema 4.10.2 (verificado). Los 6 `AR-Koha-Patron` NO se modificaron.
    - **Handling del outage .135 (DT-9/DT-11):** como `connectorErrorCriticality` no degrada el `IllegalArgumentException` interno del conector Koha, se puso Koha en **maintenance mode** (flag operacional, no GitOps) para que el backfill (y los recomputes de duales) no aborten mientras .135 está caído. **Revertir a `operational` + `reconcile-koha-daily` cuando .135 recupere** — ver `docs/runbooks/koha-maintenance-135-outage-2026-07-10.md`.
