...
tok=token()
fields=("id,displayName,userPrincipalName,mail,accountEnabled,createdDateTime,signInActivity,"
        "proxyAddresses,givenName,surname,department,officeLocation,jobTitle,city,state,"
        "companyName,usageLocation,streetAddress,employeeId,lastPasswordChangeDateTime,accountEnabled")
...
while url:
    ...
    try:
        ...
        print("HTTP",e.code,e.read()[:300].decode(),file=sys.stderr); break
    d=json.load(r)
    for u in d.get("value",[]):
        ...
        out.append({
            ...
            "department":u.get("department"),"office":u.get("officeLocation"),
            "jobTitle":u.get("jobTitle"),"city":u.get("city"),"state":u.get("state"),
            "company":u.get("companyName"),"usageLocation":u.get("usageLocation"),
            "street":u.get("streetAddress"),"employeeId":u.get("employeeId"),"pwdChange":u.get("lastPasswordChangeDateTime")
        })
    pages+=1
    if pages%20==0: print("...%d users en %ds"%(len(out),time.time()-t0),file=sys.stderr)
