...
| `extension/upeu:lambDocNum` (clave doc type-aware, prefijo `CE:`/`PP:`) | **`trabajadores` `strong` (ÚNICO con empleo vivo)**; `estudiantes`/`egresados` `strong`+`condition liveAffiliationWorker==null` | — | — | Single-valued. Las 3 fuentes canonicalizan vía `FunctionLibrary sb-document-normalizer.toCanonicalDocNumber` → output byte-idéntico para igual `(num,type)`. Desempate worker evita 2 strong en conflicto. Corrección 2026-06-29 (colisión CE vs DNI) |
| `extension/upeu:lambDocType` (código tipo LAMB) | **`trabajadores` `strong` (con empleo vivo)**; `estudiantes`/`egresados` `strong`+`condition liveAffiliationWorker==null` | — | — | Mismo desempate. ⚠️ Limitación: ELISEO etiqueta extranjeros como DNI → en worker-CE el tipo puede quedar mal (remediación pendiente: MOISES>ELISEO para el tipo). Ver `docs/runbooks/lambDocNum-collision-fix-2026-06-29.md` |
